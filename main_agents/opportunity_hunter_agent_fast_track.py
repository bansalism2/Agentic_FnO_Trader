...
opportunity_analysis_task = Task(
    ...
    **INTRADAY-ONLY CONSTRAINTS:**
    ...
    1. **OPTIMIZED ANALYSIS APPROACH - USE FAST-TRACK FIRST:**
       
       **STEP 1: Emergency Fast-Track Check (Use optimized_market_analysis() tool)**
       - This automatically runs emergency_fast_track() and quick_opportunity_check()
       - If it returns "NO_OPPORTUNITY", immediately output "NO CLEAR OPPORTUNITY - WAIT TO PRESERVE CAPITAL"
       - If it returns emergency signal (VIX spike, extreme IV), proceed directly to strategy recommendation
       - If it returns "PROCEED_TO_DETAILED", continue with detailed analysis below
       
       **STEP 2: Detailed Analysis (Only if Step 1 indicates "PROCEED_TO_DETAILED")**
       - **FIRST**: Get global market conditions using get_global_market_conditions()
       ...
       **CRITICAL**: ALWAYS start with optimized_market_analysis() tool. Do not skip this step.
