#!/usr/bin/env python3
"""
AlgoTrade - Opportunity Hunter Agent (ENHANCED WITH CRITICAL SAFETY FIXES)
=================================================================================

CRITICAL SAFETY FIXES IMPLEMENTED:
==================================

1. **EMERGENCY EXECUTION SAFETY** ✅ FIXED
   - REMOVED: Dangerous IV-only emergency execution logic
   - ADDED: Multi-factor safety checks (market regime, trend strength, technical analysis)
   - NEW: safe_emergency_check() function with 5 critical safety validations
   - RESULT: Prevents catastrophic losses in trending markets

2. **IV PERCENTILE OVER-RELIANCE** ✅ FIXED
   - REMOVED: Strategy selection based solely on IV percentile
   - ADDED: Multi-factor strategy determination (regime + trend + technical)
   - NEW: determine_strategy_from_analysis() with 4 critical checks
   - RESULT: Avoids selling premium in strong trending markets

3. **PCR ANALYSIS CONTEXT** ✅ FIXED
   - REMOVED: Absolute PCR value interpretation
   - ADDED: Contextual PCR analysis based on market regime
   - NEW: PCR interpretation varies by bull/bear/ranging markets
   - RESULT: Prevents false signals in different market conditions

4. **VOLATILITY TERM STRUCTURE** ✅ ADDED
   - NEW: analyze_volatility_term_structure() function
   - IDENTIFIES: Calendar spread opportunities
   - DETECTS: Backwardation vs Contango conditions
   - RESULT: Better expiry selection and strategy opportunities

5. **TECHNICAL INTEGRATION** ✅ ADDED
   - NEW: integrate_technical_with_options() function
   - ENSURES: Strike selection based on support/resistance levels
   - PREVENTS: Selling puts below support or calls above resistance
   - RESULT: Better risk management and strike selection

SAFETY IMPROVEMENTS:
===================
- Emergency execution now requires 5 safety checks to pass
- Strategy selection considers market regime, trend strength, and technical signals
- PCR analysis is context-aware and market-regime dependent
- Strike selection integrates technical support/resistance levels
- Term structure analysis identifies optimal expiry selection

RISK REDUCTION:
==============
- Eliminates catastrophic losses from selling premium in trending markets
- Prevents false emergency signals based on IV alone
- Reduces PCR misinterpretation in different market conditions
- Improves strike selection for better risk management
- Identifies calendar spread opportunities for better returns
...
def emergency_fast_track():
    ...
    try:
        ...
        if iv_analysis.get('status') == 'SUCCESS':
            ...
            # SAFE EMERGENCY DECISION LOGIC - Multi-factor analysis instead of IV-only
            emergency_signal = 'NO_EMERGENCY'
            
            # Only proceed with emergency check if IV conditions are met
            if current_iv > 0.30 and iv_percentile > 0.8:
                print(f"🔍 IV CONDITIONS MET: Checking for safe emergency execution...")
                
                # Get market structure and technical analysis for safe check
                try:
                    # Get market regime analysis
                    regime_analysis = detect_market_regime_wrapper()
                    market_regime = regime_analysis.get('classification', 'UNKNOWN')
                    ...
                    market_structure = {
                        ...
                        'support_levels': [spot_price * 0.98, spot_price * 0.96],  # Simplified
                        'resistance_levels': [spot_price * 1.02, spot_price * 1.04]  # Simplified
                    }
                    
                    # Prepare IV conditions
                    iv_conditions = {
                        'iv_percentile': iv_percentile,
                        'iv_ratio': current_iv / realized_vol if realized_vol else 0
                    }
                    
                    # SAFE EMERGENCY CHECK
                    safety_result = safe_emergency_check(iv_conditions, market_structure, technical_analysis)
                    
                    if safety_result.get('emergency_allowed', False):
                        emergency_signal = 'HIGH_IV_SAFE_EMERGENCY'
                        print(f"✅ SAFE EMERGENCY: All safety checks passed")
                        print(f"   Recommended Strategy: {safety_result.get('recommended_strategy', 'Iron Condor')}")
                else:
                        emergency_signal = 'NO_EMERGENCY_SAFETY_BLOCKED'
                        print(f"❌ EMERGENCY BLOCKED: {safety_result.get('reason', 'Safety check failed')}")
                        
                except Exception as e:
                    print(f"❌ Safety check failed: {e}")
                    emergency_signal = 'NO_EMERGENCY_SAFETY_ERROR'
            else:
                print(f"📊 IV CONDITIONS NOT MET: No emergency execution needed")
                print(f"   Current IV: {current_iv:.4f} (threshold: 0.30)")
                print(f"   IV Percentile: {iv_percentile:.1%} (threshold: 80%)")
            ...
            if emergency_signal.startswith('HIGH_IV'):
                ...
                try:
                    ...
                    if market_regime == 'BULL_MARKET':
                        if pcr_value > 1.2:
                            print(f"📊 PCR CONTEXT: High PCR ({pcr_value:.2f}) in bull market = profit-taking, not bearish")
                            # Don't block emergency in this case
                        elif pcr_value < 0.6:
                            print(f"❌ PCR BLOCK: Low PCR ({pcr_value:.2f}) in bull market = genuine bearish signal")
                            emergency_signal = 'NO_EMERGENCY_PCR_BLOCKED'
                    
                    elif market_regime == 'BEAR_MARKET':
                        if pcr_value > 1.5:
                            print(f"📊 PCR CONTEXT: High PCR ({pcr_value:.2f}) in bear market = potential capitulation")
                            # Don't block emergency in this case
                        elif pcr_value < 0.8:
                            print(f"❌ PCR BLOCK: Low PCR ({pcr_value:.2f}) in bear market = genuine bullish signal")
                            emergency_signal = 'NO_EMERGENCY_PCR_BLOCKED'
                    
                    else:  # RANGING or UNKNOWN market
                        if pcr_signal == 'STRONG_BUY' or pcr_extreme == 'EXTREME_BULL':
                            print(f"❌ PCR BLOCK: Strong bullish PCR in ranging market")
                            emergency_signal = 'NO_EMERGENCY_PCR_BLOCKED'
                            
                except Exception as e:
                    print(f"⚠️  PCR context analysis failed: {e}")
...
def determine_strategy_from_analysis(iv_rank, regime, intraday_tech, daily_tech, comprehensive):
    """MULTI-FACTOR STRATEGY DETERMINATION - Not just IV percentile"""
    try:
        print(f"🎯 MULTI-FACTOR STRATEGY DETERMINATION:")
        ...
        trend_strength = abs(intraday_tech.get('rsi', 50) - 50) / 50  # 0 to 1 scale
        ...
        if regime_class in ['TRENDING_BULL', 'TRENDING_BEAR']:
            if trend_strength > 0.6:  # Strong trend
                print(f"   ❌ STRONG TREND DETECTED: Avoid premium selling strategies")
                if regime_class == 'TRENDING_BULL' and iv_percentile < 30:
                    return 'Long Call'  # Only buy calls in strong bull trend with low IV
                elif regime_class == 'TRENDING_BEAR' and iv_percentile < 30:
                    return 'Long Put'   # Only buy puts in strong bear trend with low IV
                else:
                    return 'WAIT - Strong trend with high IV = dangerous'
        
        # CRITICAL CHECK 2: Premium Selling Opportunities (Multi-factor)
        if iv_percentile > 45:  # High IV for premium selling
            if regime_class in ['RANGING', 'COMPRESSED', 'NEUTRAL']:
                if trend_strength < 0.4:  # Low trend strength = good for premium selling
                    print(f"   ✅ PREMIUM SELLING OPPORTUNITY: High IV + Range-bound + Low trend")
                    if regime_class == 'RANGING':
                return 'Short Strangle'
                    else:
                return 'Iron Condor'
            else:
                    print(f"   ⚠️  HIGH TREND STRENGTH: Consider defined risk strategies")
                    if regime_class == 'TRENDING_BULL':
                        return 'Bull Put Spread'  # Defined risk bullish
                    elif regime_class == 'TRENDING_BEAR':
                        return 'Bear Call Spread'  # Defined risk bearish
                    else:
                        return 'Iron Condor'  # Conservative choice
        
        # CRITICAL CHECK 3: Low IV Opportunities (Multi-factor)
        if iv_percentile < 30:  # Low IV
            if trend_strength > 0.5:  # Strong directional signal needed
            if intraday_signal in ['STRONG_BUY', 'STRONG_SELL'] and daily_signal in ['STRONG_BUY', 'STRONG_SELL']:
                    print(f"   ✅ LOW IV + STRONG SIGNAL: Long strategy opportunity")
                if intraday_signal == 'STRONG_BUY' and daily_signal == 'STRONG_BUY':
                    return 'Long Call'
                elif intraday_signal == 'STRONG_SELL' and daily_signal == 'STRONG_SELL':
                    return 'Long Put'
                    else:
                        return 'Long Straddle'  # Mixed signals but strong movement expected
                else:
                    print(f"   ⚠️  LOW IV + WEAK SIGNAL: Wait for stronger confirmation")
                    return 'WAIT - Low IV but weak directional signals'
        
        # CRITICAL CHECK 4: Medium IV Range (Balanced approach)
        if 30 <= iv_percentile <= 45:
            if regime_class in ['RANGING', 'COMPRESSED']:
                print(f"   ✅ MEDIUM IV + RANGE-BOUND: Conservative premium selling")
                return 'Iron Condor'  # Conservative choice
            elif trend_strength < 0.3:
                print(f"   ✅ MEDIUM IV + LOW TREND: Premium selling opportunity")
                return 'Short Strangle'
            else:
                print(f"   ⚠️  MEDIUM IV + MODERATE TREND: Wait for better conditions")
                return 'WAIT - Medium IV with moderate trend = unclear edge'
        
        print(f"   ❌ NO CLEAR OPPORTUNITY: Insufficient edge for any strategy")
        return 'WAIT - No clear opportunity'
        
    except Exception as e:
        print(f"❌ Multi-factor strategy determination failed: {e}")
