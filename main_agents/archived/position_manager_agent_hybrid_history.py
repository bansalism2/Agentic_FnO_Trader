...
current_datetime = datetime.now(IST_TIMEZONE).strftime('%Y-%m-%d %H:%M:%S')
...
try:
    ...
    print("✅ Successfully imported portfolio_history")
except ImportError as e:
    print(f"❌ Warning: Could not import portfolio_history: {e}")
...
def exit_complete_strategy(trade_id, exit_reason):
    """
    Exit ALL legs of a strategy together (atomic exit).
    """
    from core_tools.execution_portfolio_tools import execute_options_strategy
    active_trades = get_active_trades()
    if trade_id not in active_trades:
        print(f"❌ Trade ID {trade_id} not found in active trades!")
        return {"status": "ERROR", "message": "Trade ID not found"}
    strategy = active_trades[trade_id]
    exit_legs = []
    ...
    print(f"🚨 Exiting complete strategy {trade_id} ({strategy.get('strategy_name', 'Unknown')}) due to: {exit_reason}")
    result = execute_options_strategy(exit_legs, order_type="Closing")
    update_trade_status(trade_id, "CLOSED", exit_reason)
    return result

def square_off_positions(positions):
    """
    Square off all open positions by exiting complete strategies atomically.
    """
    print("\n⚠️  Time > 3:15 PM: Squaring off all open positions (strategy-aware)...")
    # Build a mapping from symbol to trade_id
    active_trades = get_active_trades()
    symbol_to_trade_id = {}
    for trade_id, trade in active_trades.items():
        for leg in trade.get('legs', []):
            symbol_to_trade_id[leg['symbol']] = trade_id
    # Find all unique trade_ids for the positions to be closed
    trade_ids_to_exit = set()
    for pos in positions:
        symbol = pos.get('symbol') or pos.get('tradingsymbol')
        trade_id = symbol_to_trade_id.get(symbol)
        if trade_id:
            trade_ids_to_exit.add(trade_id)
        else:
            print(f"⚠️  No trade_id found for symbol {symbol}, skipping atomic exit.")
    # Exit each strategy atomically
    for trade_id in trade_ids_to_exit:
        exit_complete_strategy(trade_id, exit_reason="Square-off (time or emergency)")
    print("✅ All strategies squared off.")

# === Trend-Based Binary Exit Logic ===
def analyze_strategy_for_binary_exit(trade_id, strategy, trend_issue, pnl_history):
    """
    Returns ('FULL_EXIT' or 'HOLD', reason_string) based on trend analysis and binary exit criteria.
    Args:
        trade_id: str
        strategy: dict (from active_trades)
        trend_issue: str or None (from detect_trend_patterns)
        pnl_history: list of dicts [{timestamp, pnl_pct}]
    """
    # Helper: get last and first P&L, duration, failed recoveries
    if not pnl_history or len(pnl_history) < 2:
        return 'HOLD', 'Insufficient history for trend analysis'
    pnl_now = pnl_history[-1]['pnl_pct']
    pnl_start = pnl_history[0]['pnl_pct']
    t_start = pnl_history[0]['timestamp']
    t_now = pnl_history[-1]['timestamp']
    from datetime import datetime
    dt_start = datetime.fromisoformat(t_start)
    dt_now = datetime.fromisoformat(t_now)
    duration_min = (dt_now - dt_start).total_seconds() / 60
    # Count failed recoveries: number of times pnl went from negative to more negative after a recovery attempt
    failed_recoveries = 0
    below_zero = False
    for i in range(1, len(pnl_history)):
        prev = pnl_history[i-1]['pnl_pct']
        curr = pnl_history[i]['pnl_pct']
        if prev < 0 and curr > prev:
            below_zero = True  # recovery attempt
        if below_zero and curr < prev:
            failed_recoveries += 1
            below_zero = False
    # Check theta-positive (assume strategy dict has 'theta_positive' or fallback False)
    theta_positive = strategy.get('theta_positive', False)
    # Check if time decay is not helping (pnl not improving for theta-positive)
    time_decay_failure = theta_positive and all(p['pnl_pct'] < pnl_now for p in pnl_history[:-1])
    # --- Trailing Profit Drawdown Logic (lowered to 20%) ---
    peak_pnl = max(p['pnl_pct'] for p in pnl_history)
    if peak_pnl > 0:  # Only consider if there was a profit
        drawdown = (peak_pnl - pnl_now) / peak_pnl
        if drawdown > 0.2:  # 20% drawdown from peak
            return 'FULL_EXIT', f'Profit drawdown exceeded 20% from peak ({peak_pnl:.2f}% → {pnl_now:.2f}%)'
    # --- Decision Logic ---
    # 1. Profit Erosion: Profitable (>10%) declining for 45+ min, now near breakeven
    if pnl_start > 10 and pnl_now < 2 and duration_min >= 45 and pnl_now < pnl_start:
        return 'FULL_EXIT', 'Profit erosion: Profitable but declining for 45+ min, now near breakeven'
    # 2. Loss Acceleration: Loss accelerating beyond -25% with clear downward trend
    if pnl_now < -25 and pnl_now < pnl_start and trend_issue and 'loss acceleration' in trend_issue.lower():
        return 'FULL_EXIT', 'Loss acceleration: Loss > -25% and worsening'
    # 3. Failed Recovery: Negative for 60+ min, 2+ failed recovery attempts
    if pnl_now < 0 and duration_min >= 60 and failed_recoveries >= 2:
        return 'FULL_EXIT', 'Failed recovery: Negative for 60+ min, 2+ failed recoveries'
    # 4. Time Decay Failure: Theta-positive strategy not benefiting from time decay
    if time_decay_failure:
        return 'FULL_EXIT', 'Time decay failure: Theta-positive but not benefiting'
    # 5. Normal Fluctuation: All other cases
    return 'HOLD', 'Normal fluctuation or no exit criteria met'
    ...
    try:
        ...
        if positions_result.get('status') == 'SUCCESS':
            ...
            for position in nifty_positions:
                ...
                # Use symbol as trade_id for now; can be improved if trade_id available
                strategy_pnl_dict[symbol] = {'pnl_pct': pnl_percentage, 'strategy_name': 'Unknown'}
                print(f"   📊 {symbol}: Qty={quantity}, P&L=₹{pnl:.2f} ({pnl_percentage:.2f}%)")
            # === Portfolio History Integration ===
            total_portfolio_pnl_pct = (total_pnl / total_exposure) * 100 if total_exposure > 0 else 0
            record_portfolio_snapshot(total_portfolio_pnl_pct, strategy_pnl_dict)
            trend_issues = detect_trend_patterns()  # dict: {trade_id: issue}
            if trend_issues:
                print(f"⚠️  Trend Issues Detected: {trend_issues}")

            # === Trend-Based Binary Exit Integration ===
            print("\n🧠 STEP 1B: Trend-Based Binary Exit Decisions")
            active_trades = get_active_trades()
            from core_tools.portfolio_history import get_strategy_trend_history
            for trade_id, strategy in active_trades.items():
                trend_issue = trend_issues.get(trade_id)
                pnl_history = get_strategy_trend_history(trade_id)
                decision, reason = analyze_strategy_for_binary_exit(trade_id, strategy, trend_issue, pnl_history)
                print(f"   [Trend Decision] {trade_id}: {decision} ({reason})")
                if decision == 'FULL_EXIT':
                    exit_complete_strategy(trade_id, reason)
            ...
            current_time_obj = datetime.now(IST_TIMEZONE).time()
...
# Add to run_ultra_conservative_position_manager_hybrid, after direct position analysis and before trend-based exits
    # === Portfolio-Level Profit Target Exit ===
    # If total portfolio profit for the day exceeds ₹5,000, square off all positions and block further trades
    if total_pnl > 5000:
        print(f"🚨 PROFIT TARGET REACHED: Total P&L = ₹{total_pnl:.2f} > ₹5,000. Squaring off all positions and blocking further trades for the day.")
        square_off_positions(nifty_positions)
        # Create a marker file to block further trades for the day
        with open("/tmp/algotrade_no_more_trades_today", "w") as f:
            f.write(f"Profit target hit at {datetime.now(IST_TIMEZONE).isoformat()}\n")
        return {
            'status': 'SUCCESS',
            'decision': 'PROFIT_TARGET_EXIT',
            'analysis_type': 'DIRECT_ONLY',
            'positions_count': len(positions),
            'nifty_positions_count': len(nifty_positions),
            'total_pnl': total_pnl,
            ...
            'current_spot_price': current_spot,
            'position_analysis': position_analysis,
            'recommendation': 'PROFIT TARGET EXIT: All positions squared off, no further trades today',
            'timestamp': datetime.now(IST_TIMEZONE).isoformat()
        }
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print(f"🏁 ULTRA-CONSERVATIVE POSITION MANAGER (HYBRID) COMPLETED - {datetime.now(IST_TIMEZONE).strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*80)
